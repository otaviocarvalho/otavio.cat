...
**[OpenHands](https://openhands.dev/)** (formerly OpenDevin, [ICLR 2025 paper](https://arxiv.org/abs/2407.16741)) coordinates agents via an append-only **event stream** where each observation links back to the action that caused it, creating a happens-before relation. Agents read the stream, produce actions, and delegate work hierarchically via fork-join. Architecturally the closest thing to a log-structured tuple space, though it lacks associative matching and atomic consumption.
